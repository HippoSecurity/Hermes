...
function _M.sys_fetch_name()
    -- body
    local sys = ngx.shared["sys"]

    local name, _ = sys:get(KEY_CONFIG_ .. "NAME")

    ngx.log(ngx.DEBUG, "get edge name: ", name)
...
function _M.init()
    ...
    if config.mid and config.addr and config.name then
        sys_cache_store(KEY_CONFIG_ .. "MID", config.mid) 
        sys_cache_store(KEY_CONFIG_ .. "ADDR", config.addr)
        sys_cache_store(KEY_CONFIG_ .. "NAME", config.name)
        return 
    end
